...
import {
  extensionForMime,
  MAX_WIDE_FORMAT_OPTION_IMAGE_BYTES,
  uploadWideFormatOptionImage,
} from "@/lib/media-storage";

// Admin-only upload for a wide-format option's preview photo (see
// /admin/catalog/[id] → WideFormatOptionsEditor) — same shape as
// /api/admin/cover-media.
...
export async function POST(request: Request) {
  ...
  if (file.size > MAX_WIDE_FORMAT_OPTION_IMAGE_BYTES) {
    return Response.json(
      { error: `Файл слишком большой (${(file.size / 1024 / 1024).toFixed(1)} МБ). Максимум ${(MAX_WIDE_FORMAT_OPTION_IMAGE_BYTES / 1024 / 1024).toFixed(0)} МБ.` },
      { status: 400 },
    );
  }
  ...
  let url: string;
  try {
    url = await uploadWideFormatOptionImage(buffer, filename, file.type);
  } catch (error) {
    console.error("uploadWideFormatOptionImage failed:", error);
