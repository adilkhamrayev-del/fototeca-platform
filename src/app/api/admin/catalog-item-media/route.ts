...
import {
  extensionForMime,
  MAX_CATALOG_ITEM_IMAGE_BYTES,
  uploadCatalogItemImage,
} from "@/lib/media-storage";

// Admin-only upload for a catalog item's own card photo (see
// /admin/catalog/[id]) — same shape as /api/admin/cover-media, just for the
// item's own cover_image_url rather than a cover option's.
...
export async function POST(request: Request) {
  ...
  if (file.size > MAX_CATALOG_ITEM_IMAGE_BYTES) {
    return Response.json(
      { error: `Файл слишком большой (${(file.size / 1024 / 1024).toFixed(1)} МБ). Максимум ${(MAX_CATALOG_ITEM_IMAGE_BYTES / 1024 / 1024).toFixed(0)} МБ.` },
      { status: 400 },
    );
  }
  ...
  let url: string;
  try {
    url = await uploadCatalogItemImage(buffer, filename, file.type);
  } catch (error) {
    console.error("uploadCatalogItemImage failed:", error);
